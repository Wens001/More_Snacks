...
namespace RejectedGames
{
	...
	{
		[MenuItem("MyTools/PlayerPrefs查看器")]
		static void Init()
		{
			EditorWindow.GetWindow(typeof(APPWindow)).titleContent = new GUIContent("PlayerPrefs查看器");
		}
		...
		void OnGUI()
		{
			...
			EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
			{
				...
				if (GUILayout.Button(new GUIContent("排序   " + (sortAscending ? "▼" : "▲"), "将排序更改为 " + (sortAscending ? "降序" : "升序")), EditorStyles.toolbarButton, GUILayout.ExpandWidth(false)))
				{
					OnChangeSortModeClicked();
				}

				if (GUILayout.Button(new GUIContent("选项", "包含其他功能，如 \"添加新条目\" and \"删除所有条目\" "), EditorStyles.toolbarDropDown, GUILayout.ExpandWidth(false)))
				{
					GenericMenu options = new GenericMenu();
					options.AddItem(new GUIContent("新条目..."), false, OnNewEntryClicked);
					options.AddSeparator("");
					options.AddItem(new GUIContent("导入..."), false, OnImport);
					options.AddItem(new GUIContent("导出选定项..."), false, OnExportSelected);
					options.AddItem(new GUIContent("导出所有条目"), false, OnExportAllClicked);
					options.AddSeparator("");
					options.AddItem(new GUIContent("删除所选条目"), false, OnDeleteSelectedClicked);
					options.AddItem(new GUIContent("删除所有条目"), false, OnDeleteAllClicked);
				...
				{
					GenericMenu options = new GenericMenu();
					options.AddItem(new GUIContent("全部"), SearchFilter == SearchFilterType.All, OnSearchAllClicked);
					options.AddItem(new GUIContent("Key"), SearchFilter == SearchFilterType.Key, OnSearchKeyClicked);
					options.AddItem(new GUIContent("Value (仅字符串)"), SearchFilter == SearchFilterType.Value, OnSearchValueClicked);
				...
				{
					string refreshTooltip = "所有条目是否应自动刷新 " + UpdateIntervalInSeconds + " 秒?";
					AutoRefresh = GUILayout.Toggle(AutoRefresh, new GUIContent("自动刷新 ", refreshTooltip), EditorStyles.toolbarButton, GUILayout.ExpandWidth(false), GUILayout.MinWidth(75));
				}

				if (GUILayout.Button(new GUIContent(RefreshIcon, "强制刷新，可能需要几秒钟."), EditorStyles.toolbarButton, GUILayout.ExpandWidth(false), GUILayout.ExpandHeight(false)))
				{
			...
			if (showNewEntryBox)
			{
				GUILayout.BeginHorizontal(GUI.skin.box);
				{
					...
					GUILayout.BeginVertical(GUILayout.Width(1));
					{
						...
						if (GUILayout.Button(new GUIContent(AddIcon, "添加新的键值.")))
						{
				...
				EditorGUILayout.BeginVertical();
				{
					for (int i = 0; i < filteredPpeList.Count; i++)
					{
						if (filteredPpeList[i].Value != null)
						{
							...
							{
								filteredPpeList[i].IsSelected = GUILayout.Toggle(filteredPpeList[i].IsSelected, new GUIContent("", "切换选择."), filteredPpeList[i].HasChanged ? boldToggleStyle : EditorStyles.toggle, GUILayout.MaxWidth(20), GUILayout.MinWidth(20), GUILayout.ExpandWidth(false));
								...
								if (GUILayout.Button(new GUIContent(SaveIcon, "保存对此值所做的更改."), GUILayout.ExpandWidth(false)))
								{
									filteredPpeList[i].SaveChanges();
								}

								if (GUILayout.Button(new GUIContent(UndoIcon, "放弃对此值所做的更改."), GUILayout.ExpandWidth(false)))
								{
								...
								if (GUILayout.Button(new GUIContent(DeleteIcon, "删除此键值."), GUILayout.ExpandWidth(false)))
								{
		...
		private string[] GetAllKeys()
		{
			...
			if (Application.platform == RuntimePlatform.WindowsEditor)
				...
				Debug.LogError("检测到不支持的平台，请联系support@rejected-games.com并通知我们.");
