...
public class UVLens : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        // Reset view button.
        GUILayout.BeginArea(new Rect(position.size - Vector2.one * 50, Vector2.one * 50));
        if (GUILayout.Button(new GUIContent("↺", "Reset view"), GUILayout.Width(40),GUILayout.Height(40)))
            ResetView();
        GUILayout.EndArea();
    }
    ...
    [MenuItem("MyTools/Mesh/显示网格UV")]
    [MenuItem("CONTEXT/MeshFilter/UV Lens")]
    [MenuItem("CONTEXT/SkinnedMeshRenderer/UV Lens")]
    static void Open()
    {
        EditorWindow window = GetWindow(typeof(UVLens));
        window.titleContent = new GUIContent("网格UV");
